...
- Hidden native `sessions_spawn` (default runtime, `visible` omitted/false) does not wait for child completion. After context preparation and Gateway dispatch, startup acceptance returns `{ status: "accepted", runId, childSessionKey, sessionId }`. `sessionId` is the durable child UUID for those direct subagent accepts (prefer it over parsing `childSessionKey`). `visible=true` and supported `runtime="acp"` accepts omit `sessionId`; `sessionKey` remains collector-launch only. Cloud-worker spawns can wait for provisioning before returning this receipt.
