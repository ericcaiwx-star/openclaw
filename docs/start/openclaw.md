...
- Delivery coverage on this contract: outbound `message.action` attachment normalization + **Telegram** direct reply/native-command delivery. Configured directories use `resolveAgentScopedOutboundMediaAccess`. Other gateway send paths and channel reply paths (for example Discord/Mattermost) still use the generic managed-root helper and intentionally do **not** ambient-grant these directories until they are wired through the same resolver.
- Telegram compatibility: omitting this setting or using `[]` preserves existing managed-root and active-workspace selection at the Telegram delivery boundary. This does not bypass earlier checks: model-generated `MEDIA:` attachments already undergo sender/group read-policy validation during staging. A nonempty list opts both direct replies and native commands into the stricter shared policy at delivery too: when sender/group policy denies `read`, only managed outbound media is allowed, not workspace files or the configured directories. There is no fallback to the previous roots after a denial.
